...
# Helper function to color text inside square brackets
def color_brackets(text, color_code="\033[94m", reset_code="\033[0m"):
    # This regex finds any part of the text enclosed in square brackets
    return re.sub(r'(\[.*?\])', lambda m: f"{color_code}{m.group(1)}{reset_code}", text)
...
def display_progress(message, percent, is_webui, progress=None):
    if is_webui:
        ...
        # Only the text in square brackets will be colored
        colored_message = color_brackets(message)
...
def raise_exception(error_msg, is_webui):
    if is_webui:
        ...
        colored_message = color_brackets(message)
        raise Exception(colored_message)
...
def song_cover_pipeline(song_input, voice_model, pitch_change, keep_files,
    ...
    try:
        ...
        pitch_change = pitch_change * 12 
        ai_vocals_path = os.path.join(song_dir, f'output_{os.path.splitext(os.path.basename(orig_song_path))[0]}_{voice_model}.wav')
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='RVC Inference CLI.', add_help=True)
